...
it('can add the star name and star symbol properly', async() => {
    ...
    assert.equal('Decentralized Star', tokenName);
    assert.equal('DS', tokenSymbol);
});
...
it('lets a user transfer a star', async() => {
    ...
    let user2 = accounts[2];
    let starId = 33;
    await instance.createStar('Star 33', starId, {from: user1});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user = accounts[1];
    let starId = 44;
    await instance.createStar('Star 44', starId, {from: user});

    let starName = await instance.lookUptokenIdToStarInfo(starId);
    assert.equal('Star 44', starName);
});
